					  ...
					  LPVOID reserved)
{
	switch (callReason)
	{
		case DLL_PROCESS_ATTACH:
		{
			...
			// Create the console window to display the logs
			RedirectToConsole();
		...
		case DLL_PROCESS_DETACH:
		{
			...
			// Prevent console from exit immediately after the app is closed
			int num = 0;
			cout << "Enter any number to exit the app: ";
			cin >> num;
		...
		if (isSuccess)
		{
...

			{
				...
				if (strstr(undName, "std::") ||
					strstr(undName, "GetThisMessageMap") ||
					strstr(undName, "GetMessageMap") ||
					strstr(undName, "AssertValid") ||
					strstr(undName, "GetRuntimeClass") ||
					strstr(undName, "_GetBaseClass") ||
					strstr(undName, "SupportsRestartManager") ||
					strstr(undName, "OnDraw") || 
					strstr(undName, "GetThisClass") ||
					strstr(undName, "GetDocument") ||
					strstr(undName, "CFrameWnd") ||
					strstr(undName, "CMainFrame") ||
					strstr(undName, "CView") ||
					strstr(undName, "CDocTemplate") ||
					strstr(undName, "CRuntimeClass"))
				{
					// Skip the symbol
				}
				else
...
void RedirectToConsole()
{
	// If console is not there, allocate and get the handle of the console
	if(!g_hConsole)
	{
		BOOL isSuccess = ::AllocConsole();
		g_hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
		::SetConsoleTextAttribute(g_hConsole, FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED);	// White text
		SetConsoleTitle(TEXT("Profiler"));
		FILE *conin, *conout;
